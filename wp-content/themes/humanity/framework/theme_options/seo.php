                            ...
                            <div class="bpanel-option-set">
                                ...
                                <ul class="menu-to-edit drag-list">
                                	<?php $t = is_array(dttheme_option('seo',$option['id'])) ? dttheme_option('seo',$option['id']) : array();
										 if(sizeof($t) > 0):
										 	foreach($t as $key):?>
                                                <li><label><input  type="checkbox" name="<?php echo esc_attr($option["name"]."[".$option['id']."][]");?>" value="<?php echo esc_attr($key);?>"
                                                checked="checked"/><?php echo esc_html($option["options"][$key]);?></label></li>                                            
									<?php		 unset($option["options"][$key]);
											endforeach;
										 endif;?>
									<?php foreach($option["options"] as $key => $value): ?>
                                        <li><label> <input type="checkbox" name="<?php echo esc_attr($option["name"]."[".$option['id']."][]");?>" value="<?php echo esc_attr($key);?>" />
                                            <?php echo esc_html($value);?></label></li>
                                    <?php endforeach;?>
                                </ul>
                            </div>
                            <div class="hr"> </div>
        	...
        	<div class="bpanel-box">
				...
				foreach($others as $option):
                    ...
                    <div class="<?php echo esc_attr($last);?>">
               	 		<div class="bpanel-option-set">
                        	<label>
                            	<input id="mytheme[seo][<?php echo esc_attr($option['id']);?>]" type="checkbox" name="mytheme[seo][<?php echo esc_attr($option['id']);?>]" 
                                	value="<?php echo esc_attr($option['id']);?>" <?php checked($option['id'],dttheme_option('seo',$option['id']));?> /> <?php echo esc_html($option['label']);?>
                            </label>
                            <p class="note"><?php echo esc_attr($option["tooltip"])?> </p>
                        </div>
                    </div>
		 <?php  endforeach;?>
                </div>
