...
class MY_Gallery_Widget extends WP_Widget {
	#1.constructor
	function MY_Gallery_Widget() {
		$widget_options = array("classname"=>'widget_popular_entries', 'description'=>'To list out DesignThemes Gallery items');
		parent::__construct(false,IAMD_THEME_NAME.__(' Gallery','dt_themes'),$widget_options);
	}
	...
	function form($instance) {
	    ...
	    <p><label for="<?php echo esc_attr($this->get_field_id('_post_categories')); ?>">
            ...
            <select class="widefat" id="<?php echo esc_attr($this->get_field_id('_post_categories').'[]');?>" 
           	...
           	<?php $cats = get_categories('taxonomy=dt_gallery_entries&hide_empty=1');
			foreach ($cats as $cat):
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('_excerpt')); ?>"><?php _e('Display title only or title &amp; excerpt','dt_themes');?></label>
					...
					if(1 == $_enabled_image):
                         ...
                         $portfolio_settings = is_array ( $portfolio_settings ) ? $portfolio_settings : array ();
						 	...
						 	if( array_key_exists("items_name",$portfolio_settings) ):
								...
								$image;								
								if( "video" === $item ):
									$image = "http://placehold.it/90/999999/&text=Video%20Portfolio";
                                    else:
                                        $image = $portfolio_settings['items'][0];
                                    endif;
                                else:
                                    $image = "http://placehold.it/90/999999/&text=Add%20Image";
                                endif;
