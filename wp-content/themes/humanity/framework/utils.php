...
function dttheme_load_basic_css() {
	...
	wp_enqueue_style ( 'humanity-default', get_stylesheet_uri () );
	wp_enqueue_style ( 'skin', IAMD_BASE_URL . "skins/" . $dttheme_options ['appearance'] ['skin'] . "/style.css" );
	
	
	wp_enqueue_style ( 'animations', IAMD_BASE_URL . 'css/animation.css', array (), '' );
	...
	if(dttheme_is_plugin_active('woocommerce/woocommerce.php')){
		wp_enqueue_style('stylewoo', IAMD_BASE_URL.'framework/woocommerce/style.css');
	}
}
...
function dttheme_show_sidebar($type , $id ){
	...
	if( array_key_exists("widget-area", $settings)):
		foreach ($settings['widget-area'] as $widget ) {
			...
			if (is_active_sidebar ( $id )):  dynamic_sidebar ( $id ); endif;
		}
	endif;
