...
/**
 * Display the'Documentation' and the 'Support' support links 
 * in 'Plugins -> Installed Plugins' page.
 *
...
function responsive_map_css() {
    wp_register_style('responsive_map_css', plugins_url('includes/css/rsmaps.min.css', __FILE__), false, '2.26');
...
function responsive_map_scripts() {
    ...
    if (is_ssl()) {
            wp_register_script('googlemapsapi', 'https://maps-api-ssl.google.com/maps/api/js?sensor=false&v=3.exp&libraries=adsense,places', array( 'jquery' ), null, true);
    } else {
            wp_register_script('googlemapsapi', 'http://maps.googleapis.com/maps/api/js?sensor=false&v=3.exp&libraries=adsense,places', array( 'jquery' ), null, true);
    }
    // Also register the plugin's javascript file
    wp_register_script('rsmaps', plugins_url('includes/js/rsmaps.min.js', __FILE__), array('jquery'), '3.3.3', true);
...
function admin_responsive_map_scripts($hook) {
    ...
    if (is_ssl()) {
            wp_register_script('googlemapsapi', 'https://maps-api-ssl.google.com/maps/api/js?sensor=false&v=3.exp&libraries=adsense,places', array( 'jquery' ), null, false);
    } else {
            wp_register_script('googlemapsapi', 'http://maps.googleapis.com/maps/api/js?sensor=false&v=3.exp&libraries=adsense,places', array( 'jquery' ), null, false);
    }
    wp_register_script('rsmaps', plugins_url('includes/js/rsmaps.min.js', __FILE__), array('jquery'), '2.26');
...
function responsive_map_shortcode($atts) {
    ...
    // 1) Generate a unique identifier for the map
    $mapid = rand();
    ...
    // Prepare the parameters we will send to the jQuery map library
    $maptype = $atts['maptype'];
    $logging = toBool($atts['logging']);
    $zoom = 
    ...
    createSearchControl(mapdiv.data('gmap').gmap);
    <?php } ?>
