...
function dt_new_save_layout(){
	...
	if ( get_post_meta( $dt_post_id, '_dt_builder_settings', true ) ) update_post_meta( $dt_post_id, '_dt_builder_settings', $dt_convertible_settings );
...
if ( ! function_exists('dt_generate_widget_options') ){
	function dt_generate_widget_options( $module_name, $dt_module_exact_name, $paste_to_editor_id, $module_window ){
		...
		echo '<form id="'.esc_attr( $form_id ).'" '.$form_cls.'>';
		echo '<div class="dt_settings_title">' . esc_html( $dt_module_exact_name . ' ' . __('Settings', 'dt_themes') ) . '</div>';
...
function dt_show_columoptions_panel(){
	...
	if ( ! wp_verify_nonce( $_POST['dt_load_nonce'], 'dt_load_nonce' ) ) die(-1);
	$dt_column_type = isset($_POST['dt_column_type']) ? $_POST['dt_column_type'] : '';
	$dt_column_id = isset($_POST['dt_column_id']) ? $_POST['dt_column_id'] : '';
	...
	dt_generate_columnoptions_panel($dt_column_type, $dt_column_id, $dt_column_css, $dt_column_animation_type, $dt_column_animation_delay);
	
	die();
}

if ( ! function_exists('dt_generate_columnoptions_panel') ){
	function dt_generate_columnoptions_panel( $dt_column_type, $dt_column_id, $dt_column_css, $dt_column_animation_type, $dt_column_animation_delay ){
		...
		echo '<form id="dt_column_settings">'
				...
				. '<p><label>Id</label><input type="text" class="regular-text dtthemes_option" name="dt_column_id" id="dt_column_id" value="'.$dt_column_id.'"  /></p>'			
				...
				echo
				'<p><label>Type</label><span class="selection-box"><select name="dt_column_type" id="dt_column_type" >'
					. '<option value="None">  ' . esc_html__('Select', 'dt_themes') . '  </option>';
						echo '<option value="type1" ' . selected( $dt_column_type, 'type1', false ) . '>' . esc_html__('Type 1', 'dt_themes') . ' </option>';
						echo '<option value="type2" ' . selected( $dt_column_type, 'type2', false ) . '>' . esc_html__('Type 2', 'dt_themes') . ' </option>';
				echo '</select></span></p>';
...
function dtthemes_color_picker($name, $value) {
	...
	if (( float ) $wp_version >= 3.5) :
		$output .= "<input type='text' class='dtthemes_option color-field medium' name='".esc_attr($name)."' id='".esc_attr($name)."' value='".esc_attr($value)."' />";
	 else :
		$output .= "<input type='text' class='medium color_picker_element' name='".esc_attr($name)."' id='".esc_attr($name)."' value='".esc_attr($value)."' />";
